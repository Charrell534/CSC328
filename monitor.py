...
class LogViewer:
    ...
    def __init__(self, s_pid):
        ...
        # Start server button
        self.start_btn = tk.Button(self.root, text="Open", command=self.start)
    ...
    def _populate_message_log(self):
        ...
        with open(self.server_file, 'r') as file:
            for line in file:
                ...
                self.message_log_text.delete(1.0, tk.END)  # Clear existing content
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        exit()
    log_viewer = LogViewer(sys.argv[1])
    log_viewer.run()
