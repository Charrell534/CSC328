...
class ServerApp(Connection):
    ...
    def _start(self, signum, frame):
        ...
        """
        self.write_to_log({"message": "Starting..."})
    ...
    def request_username(self, c_socket):
        ...
        """
        msg = json.dumps({"type": "message", "message": "welcome", "username": "server",
                          "timestamp": f"{self.get_time()}"})
...
if __name__ == '__main__':
    try:
        ...
        if len(sys.argv) < 3:
            print("Error incorrect number of arguments. See usage below.")
    ...
    # shouldn't hit any of these, but users ARE stupid
        ...
        print(f"Value err: {e}")
        print("Incorrect data, <host_name> must be a valid IP address and"
              " <host_port> must be a integer. See usage below.")
        print(f"Usage: {sys.argv[0]} <host_name> <host_port>")
    except OSError as e:
        print(f"OS error {e}")
        print("Incorrect data, <host_name> must be a valid IP address and "
